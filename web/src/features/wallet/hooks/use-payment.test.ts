...
describe('payment amount routing', () => {
  ...
  test('uses the dedicated BEpusdt amount calculator', async () => {
    const calls: string[] = []
    const amount = await requestPaymentAmount(80, PAYMENT_TYPES.BEPUSDT, {
      ...
      bepusdt: async (request) => {
        calls.push(`bepusdt:${request.amount}`)
        return { success: true, data: '12.5' }
      },
    })

    expect(amount).toBe(12.5)
    expect(calls).toEqual(['bepusdt:80'])
  })
})
