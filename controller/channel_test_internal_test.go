...
func TestAutomaticChannelTestDoesNotEnableAfterLocalFailure(t *testing.T) {
	...
	result := testChannel(context.Background(), &model.Channel{Type: constant.ChannelTypeMidjourney}, 1, "", "", false)
...
func TestPerformChannelTestsCountsLocalFailureAsFailure(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.AutoMigrate(&model.Channel{}))

	channel := &model.Channel{
		Type:   constant.ChannelTypeMidjourney,
		Name:   "unsupported-automatic-test",
		Status: common.ChannelStatusEnabled,
	}
	require.NoError(t, db.Create(channel).Error)

	oldRequestInterval := common.RequestInterval
	common.RequestInterval = 0
	t.Cleanup(func() {
		common.RequestInterval = oldRequestInterval
	})

	summary := performChannelTests(context.Background(), []*model.Channel{channel}, 1, false, nil)
